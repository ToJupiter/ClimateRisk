...
def table_descriptive_statistics(df, y_col, t_col, x_cols):
    ...
    vif_results = calculate_vif(df, x_cols)
    
    stats_df = stats_df.merge(vif_results.set_index('Variable'), 
                              left_index=True, 
                              right_index=True, 
                              how='left')
...
def table_covariate_balance_pearson(df, t_col, x_cols, threshold=0.1):
    """
    Assess covariate balance using Pearson correlation between treatment indicator
    and covariates (point-biserial correlation).
    """
    median_t = df[t_col].median()
    df = df.copy()
    df['treatment_indicator'] = (df[t_col] >= median_t).astype(int)
    
    balance_df = pd.DataFrame()
    
    for var in x_cols:
        if var in df.columns:
            treatment_group = df[df['treatment_indicator'] == 1][var]
            control_group = df[df['treatment_indicator'] == 0][var]
            
            t_mean = treatment_group.mean()
            c_mean = control_group.mean()
            
            # correlation, p_value = stats.pointbiserialr(df['treatment_indicator'], df[var])
            
            # Alternative: using pearsonr directly (same result)
            correlation, p_value = stats.pearsonr(df['treatment_indicator'], df[var])
            
            balanced = 'Yes' if abs(correlation) < threshold else 'No'
...
# Usage
table2 = table_covariate_balance_pearson(df_mapped, t_col, x_cols)
table2
...
def table_ml_performance(df, y_col, t_col, x_cols, cv_folds=5):
    """Match causal_analysis.py specifications exactly"""
    X = df[x_cols].values
    Y = df[y_col].values
    T = df[t_col].values
    results = []
    
    # Random Forest for Y (same as causal_analysis.py)
    ...
    # XGBoost for Y (MATCH causal_analysis.py parameters)
    xgb_y = xgb.XGBRegressor(
        ...
        reg_alpha=0.1,            # ✅ Added (Y model)
        reg_lambda=0.5,           # ✅ Added (Y model)
        random_state=42,
        verbosity=0
    )
    ...
    # Random Forest for T (same as causal_analysis.py)
    ...
    # XGBoost for T (MATCH causal_analysis.py parameters)
    xgb_t = xgb.XGBRegressor(
        ...
        subsample=0.8,            # ✅ Added
        colsample_bytree=0.8,     # ✅ Added
        reg_alpha=0,              # ✅ Added (T model - different from Y!)
        reg_lambda=1,             # ✅ Added (T model - different from Y!)
        random_state=42,
        verbosity=0
    )
...
# %% [markdown]
# ## TABLE 4: Baseline OLS Results (Deprecated)
...
# %%
def dml_estimate(df, y_col, t_col, x_cols, model_type='rf', cv_folds=5, case_num=None, include_moderators=False):
    Y = df[y_col].values
    ...
    if case_num in [3,4] or include_moderators:
        W = df['ESG'].values.reshape(-1,1)
    else:
        W = None
        
    if model_type == 'rf' or case_num in [1,3]:
...
def table_causal_estimates(df, y_col, t_col, x_cols):
    """Match causal_analysis.py case structure"""
    results = []
    
    # Case 1: No confounders, No moderator (RF)
    dml_case1 = dml_estimate(df, y_col, t_col, x_cols, model_type='rf', case_num=1)
    results.append({
        'Algorithm': 'DML - Random Forest (Case 1)',
        'ATE': dml_case1['ATE'],
        'Std_Error': dml_case1['Std_Error'],
        'CI_Lower': dml_case1['CI_Lower'],
        'CI_Upper': dml_case1['CI_Upper'],
        'P_Value': dml_case1['P_Value']
    })
    
    # Case 2: With confounders, No moderator (XGB)
    dml_case2 = dml_estimate(df, y_col, t_col, x_cols, model_type='xgb', case_num=2)
    results.append({
        'Algorithm': 'DML - XGBoost (Case 2)',
        'ATE': dml_case2['ATE'],
        'Std_Error': dml_case2['Std_Error'],
        'CI_Lower': dml_case2['CI_Lower'],
        'CI_Upper': dml_case2['CI_Upper'],
        'P_Value': dml_case2['P_Value']
    })
    
    # Case 3: No confounders, With moderator (RF)
    dml_case3 = dml_estimate(df, y_col, t_col, x_cols, model_type='rf', case_num=3)
    results.append({
        'Algorithm': 'DML - Random Forest (Case 3)',
        'ATE': dml_case3['ATE'],
        'Std_Error': dml_case3['Std_Error'],
        'CI_Lower': dml_case3['CI_Lower'],
        'CI_Upper': dml_case3['CI_Upper'],
        'P_Value': dml_case3['P_Value']
    })
    
    # Case 4: With confounders, With moderator (XGB)
    dml_case4 = dml_estimate(df, y_col, t_col, x_cols, model_type='xgb', case_num=4)
    results.append({
        'Algorithm': 'DML - XGBoost (Case 4)',
        'ATE': dml_case4['ATE'],
        'Std_Error': dml_case4['Std_Error'],
        'CI_Lower': dml_case4['CI_Lower'],
        'CI_Upper': dml_case4['CI_Upper'],
        'P_Value': dml_case4['P_Value']
    })
